...
import styles from './EngSlot.module.scss';
...
const Slot:React.FC<ISlotProps> = ({ slot }) => {
  ...
  const handleEngSelect = (slot: ISlot) => {
    if (slot.state === 'idle' || slot.state === 'selected') {
      dispatch({ type: 'SELECTED_ENGLISH', value: { ...slot, state: 'selected' } });
    }
  }
